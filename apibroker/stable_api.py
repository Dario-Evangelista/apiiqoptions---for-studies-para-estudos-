from apibroker.api import IQOptionAPI
import apibroker.constants as OP_code
import apibroker.country_id as Country
import threading
import time
import json
import logging
...
from apibroker.expiration import get_expiration_time, get_remaning_time
...
class API:
    ...
    def __init__(self, email, password, active_account_type="PRACTICE", host='iqoption.com/'):
    ...
    def connect(self, sms_code=None):
        ...
        self.api = IQOptionAPI(
            self.HOST, self.email, self.password, conta = self.active_account_type)
        check = None
    ...
    def get_subscripts(self, position, digital):
        self.api.subscribe_position_changed(
                position, digital, 6)
    def get_all_ACTIVES_OPCODE(self):
        return OP_code.ACTIVES
    
    def get_all_open(self, active, tipy):
        """
        tipy = forex, cfd, crypto, digital, turbo, binary
        """
        ALL_Asset=self.get_all_open_time()
        if ALL_Asset[tipy][active]["open"] != None:
            return True
        else:
            return None
        
    def subblitz(self, active):
        self.api.subblitz(active)
    ...
    def get_all_actives(self):
        ALL_Asset=self.get_all_init()
        todos_ativos = {}
        for dirr in ["binary", "turbo"]:
            ativos = ALL_Asset["result"].get(dirr, {}).get("actives", {})
            for id_str, info in ativos.items():
                nome_completo = info.get("name", "")
                if "." in nome_completo:
                    nome_curto = nome_completo.split(".")[1]
                    todos_ativos[nome_curto] = int(id_str)
        print(todos_ativos)

    # _________________________self.api.get_api_option_init_all() wss______________________

    def get_all_news(self):
        return self.api.option_news_initialization
    ...
    def coletar_blitz_payouts(self):
        try:
            init_data = self.get_all_init_v2()
            resultado = {}
            blitz_actives = init_data["blitz"]["actives"]
            for ativo_id, dados in blitz_actives.items():
                try:
                    payout = 100 - dados["option"]["profit"]["commission"]
                    if payout > 0:
                        self.playout[ativo_id] = payout
                        
                except Exception:
                    continue
        except Exception as e:
            print(f"[{datetime.now()}] Erro ao coletar blitz payouts: {e}")
            return {}
        # return OP_code.ACTIVES
    ...
    def reset_practice_balance(self):
        id = self.get_balance_id()
        self.api.training_balance_reset_request = None
        self.api.reset_training_balance(id)
    ...
    def get_balance_mode_SET(self):
        balance_mode = self.active_account_type
        def set_id(b_id):
...
##############################################################################################
    ...
    """def check_win(self, id_number, timeout=15):
        start_time = time.time()
        while True:
            time.sleep(1)
            try:
                response = self.api.sold_options_respond

                if response is None:
                    time.sleep(1)
                    continue

                if str(id_number) in response.get("msg", {}):
                    result = response["msg"][str(id_number)]
                    game_state = result.get("game_state")

                    if game_state == 1: 
                        win_status = result.get("win")
                        profit = result.get("profit_amount")

                        self.api.sold_options_respond["msg"].pop(str(id_number), None)
                        return win_status

            except Exception as e:
                print(f"⚠️ Erro ao buscar resultado: {e}")
    ...
    def check_win(self, id_number):
        # Aguarda até a posição ser registrada
        while True:
            time.sleep(2)
            try:
        ...
        msg = x.get('msg', {})
        win = msg.get('win')
        close_reason = msg.get('close_reason')
        source = msg.get('source', '')

        # Para digital-options, calcula usando pnl_realized
        if source == 'digital-options':
            pnl = float(msg.get('pnl_realized', msg.get('pnl', 0)))
            if pnl > 0:
                result = 'win'
            elif pnl < 0:
                result = 'lose'
        ...
        # Para binary-options ou quando 'win' existe
        if win is not None:
            if win == 'equal':
                value = 0
            elif win == 'loose':
                value = -float(msg.get('sum', 0))
            else:
                value = float(msg.get('win_amount', 0)) - float(msg.get('sum', 0))
            return win, value

        # Para posições binary-options vendidas
        invest = float(msg.get('invest', 0))
        close_profit = float(msg.get('close_profit', 0))
    ...
    def get_betinfo(self, id_number, max_retries=3):
        attempt = 0
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                logging.error(f'❌ Erro ao enviar get_betinfo: {e}')
                self.connect()
                attempt += 1
                continue

            # Aguarda resposta do WebSocket por no máximo 10 segundos
            while self.api.game_betinfo.isSuccessful is None:
                time.sleep(1)
                if time.time() - start > 10:
                    logging.error('⏰ Timeout esperando resposta do get_betinfo')
                    self.connect()
                    attempt += 1
                    break
                time.sleep(0.1)
            ...
            # Se falhou mas respondeu
            elif self.api.game_betinfo.isSuccessful is False:
                return False, None

        logging.error("❌ Falha ao obter get_betinfo após várias tentativas.")
        return False, None
    ...
    def buy(self, price, ACTIVES, ACTION, expirations, tipz=None):
        self.api.buy_multi_option = {}
        self.api.buy_successful = None
        profit = self.get_digital_payout(ACTIVES)
        ...
        try:
            ...
            pass
        self.api.buyv3(
            float(price), OP_code.ACTIVES[ACTIVES], str(ACTION), int(expirations), req_id, profit, tipz)
        start_t = time.time()
        id = None
        self.api.result = None
        while self.api.result == None or id == None:
            ...
            try:
                if "message" in self.api.buy_multi_option[req_id].keys():
                    if self.api.buy_multi_option[req_id]["message"] == 'Cannot purchase an option (the asset is not available at the moment).':
                        msg = None
                    else:
                        msg = self.api.buy_multi_option[req_id]["message"]
                    return False, msg
            except:
                pass
            try:
    ...
    def buy2(self, price, ACTIVES, ACTION, expirations, tipz=None):
        self.api.buy_multi_option = {}
        self.api.buy_successful = None
        profit = self.get_digital_payout(ACTIVES)
        req_id = str(randint(0, 10000))
    ...
    def blitz(self, price, ACTIVES, ACTION, expirations, tipz=None):
        self.api.buy_multi_option = {}
        self.api.buy_successful = None
        id = str(OP_code.ACTIVES[ACTIVES])
        profit = self.playout.get(id)
        req_id = str(randint(0, 10000))
        try:
            self.api.buy_multi_option[req_id]["id"] = None
        except:
            pass
        self.api.buyv4(
            float(price), OP_code.ACTIVES[ACTIVES], str(ACTION), int(expirations*60), req_id, profit, tipz)
        start_t = time.time()
        id = None
        self.api.result = None
        while self.api.result == None or id == None:
...
# __________________for Digital___________________
    ...
    def subscribe_strike_list(self, ACTIVE, expiration_period,type='blitz-option'):
        self.api.subscribe_instrument_quites_generated(
            OP_code.ACTIVES[ACTIVE], expiration_period, type=type)
    ...
    def get_digital_option_symbol(msg, asset_id_alvo, direcao="call", duration="1M"):
        data = msg["msg"]

        if data["asset_id"] != asset_id_alvo:
            return None, None

        instrument_index = data.get("instrument_index")

        for price in data["prices"]:
            if price["strike"] == "SPT":
                try:
                    symbol = price[direcao]["symbol"]
                    if f"T{duration.upper()}" in symbol:
                        return instrument_index, symbol
                except KeyError:
                    pass

        return None, None

    def buy_digital_spot(self, active, amount, action, duration, timeout=10):
        ...
        if action == 'put':
            ...
            logging.error('buy_digital_spot action error')
            return False, None
        ...
        if duration == 1:
            ...
            now = datetime.fromtimestamp(timestamp, tz=timezone.utc) + timedelta(minutes=1, seconds=30)
            while True:
                time.sleep(1)
                if now.minute % duration == 0 and (now.timestamp() - timestamp) > 30:
                    break
                now += timedelta(minutes=1)
            exp = int(now.timestamp())

        asset_id = OP_code.ACTIVES[active] 
        date_formatted = datetime.fromtimestamp(exp, tz=timezone.utc).strftime("%Y%m%d%H%M")
        instrument_ids = f"do{asset_id}A{date_formatted}D{datetime.fromtimestamp(exp, tz=timezone.utc).strftime('%H%M%S')}T{duration}M{action_code}SPT"
        instrument_index = self.api.instrument_index_id
        instrument_index,instrument_id = self.get_digital_option_symbol(instrument_index,asset_id,action)
        request_id = self.api.place_digital_option(instrument_id, amount, instrument_index, asset_id)
        start_time = time.time()
        while self.api.digital_option_placed_id.get(request_id) is None:
            time.sleep(1)
            if time.time() - start_time > timeout:
                logging.error("Timeout waiting for digital option placement confirmation")
                return False, None
            time.sleep(1)
    ...
    def check_win_digital_v2(self, buy_order_id):
        while True:
            order = self.get_async_order(buy_order_id)
            position_changed = order.get("position-changed", {})

            if position_changed:
                order_data = position_changed.get("msg")

                if order_data and order_data.get("status") == "closed":
                    close_reason = order_data.get("close_reason")
                    invest = order_data.get("invest", 0)

                    if close_reason == "expired":
                        close_profit = order_data.get("close_profit", 0)
                        lucro = close_profit - invest
                    elif close_reason == "default":
                        lucro = order_data.get("pnl_realized", 0)
                    else:
                        lucro = 0  
                    if lucro > 0:
                        return "win", lucro
    ...
    def logout(self):
        self.api.logout(self.host)

    def buy_digital(self, active, amount, action, duration):
        action = action.lower()
        if action == 'put':
            action = 'P'
        elif action == 'call':
        ...
        date_formated = datetime.fromtimestamp(exp, timezone.utc).strftime("%Y%m%d%H%M")
        ...
        """while self.api.digital_option_placed_id.get(request_id) is None:
            pass
        time.sleep(1)
        print(self.api.digital_option_placed_id)"""

        #digital_order_id = self.api.digital_option_placed_id.get(request_id)
        while True:
            time.sleep(1)
            if self.api.socket_option_open_digital != None:
                break
        digital_order_id = self.api.socket_option_open_digital
        if isinstance(digital_order_id, int):
            return True, digital_order_id
        else:
            return False, digital_order_id
